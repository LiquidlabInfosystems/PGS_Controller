...
PARAM_DIR=/home/rakesh/Liquidlab/Projects/PGS/working_folder/test_xml/Parameter

cd $PARAM_DIR
touch parameters.txt main_parameters.txt

cd /home/rakesh/Liquidlab/Projects/PGS/working_folder/test_xml
...
LEVEL_ID=$(cat $CONFIG_FILE | grep level_controller_ip |grep $HOST_IP| cut -d '-' -f1 | cut -d " " -f2)
...
echo level_search_keyword $LEVEL_SEARCH_KEYWORD


LEVEL_SPACES=$(cat $CONFIG_FILE |grep $LEVEL_SEARCH_KEYWORD |cut -d '"' -f2)


echo  spaces $LEVEL_SPACES
...
echo zones_details $ZONE_DETAILS
